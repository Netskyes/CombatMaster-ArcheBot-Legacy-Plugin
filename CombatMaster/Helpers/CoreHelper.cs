...
namespace CombatMaster
{
    ...
    public abstract class CoreHelper
    {
        ...
        public bool AnyBuffContains(string pattern)
        {
            return Host.me.getBuffs().Any(b => b.name.Contains(pattern));
        }
        ...
        public List<Item> GetEquippedSkills()
        {
            byte[] cellIgnores = { 26, 14, 18 };

            return Host.me.getAllEquipedItems().Where(i => !cellIgnores.Contains(i.cell) && i.db.useSkillId != 0).ToList();
        }
        ...
        public IEnumerable<Creature> GetHostiles() => Host.getCreatures().Where(c => IsAttackable(c));

        public bool IsDangerInZone(int radius)
        {
            RoundZone zone = new RoundZone(Host.me.X, Host.me.Y, radius);

            return GetHostiles().Any(c => zone.ObjInZone(c));
        }
